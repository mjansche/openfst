...
#include "fst/lib/random-weight.h"
 ...
 private:
  ...
  void TestOptimize(const Fst<Arc> &T) {
    ...
    if ((wprops & (kPath | kCommutative)) == (kPath | kCommutative)) {
      ...
      // Checks Sum(T- prune(T)) > Sum(T) times Thresold
      {
        Weight thresold = (*weight_generator_)();
        VectorFst<Arc> P;
        Prune(A, &P, thresold);
        ...
        Weight sum1 = Times(Sum(A), thresold);
        Weight sum2 = Sum(C);
  ...
  bool Equiv(const Fst<Arc> &fst1, const Fst<Arc> &fst2) {
    ...
    for (int n = 0; n < kNumRandomPaths; ++n) {
      ...
      ComposeFst<Arc> cfst1(ipath, sfst1);
      ComposeFst<Arc> pfst1(cfst1, opath);
      // Give up if there are epsilon cycles in a non-idempotent semiring
      if (!(Weight::Properties() & kIdempotent) &&
          pfst1.Properties(kCyclic, kCyclic))
        continue;
      Weight sum1 = Sum(pfst1);

      ComposeFst<Arc> cfst2(ipath, sfst2);
      ComposeFst<Arc> pfst2(cfst2, opath);
      // Give up if there are epsilon cycles in a non-idempotent semiring
      if (!(Weight::Properties() & kIdempotent) &&
          pfst2.Properties(kCyclic, kCyclic))
        continue;
      Weight sum2 = Sum(pfst2);

      if (!ApproxEqual(sum1, sum2, kTestDelta)) {
        VLOG(1) << "Sum1 = " << sum1;
        VLOG(1) << "Sum2 = " << sum2;
        return false;
      }
  ...
  // Sums all paths in an FST.
  Weight Sum(const Fst<Arc> &fst) {
  ...
  static const int kNumRandomShortest = 10;

  // Delta for equivalence tests.
  static const float kTestDelta;

  DISALLOW_EVIL_CONSTRUCTORS(WeightedTester);
};

template <class A, class WG>
const float WeightedTester<A, WG>::kTestDelta = .01;
...
class UnweightedTester<StdArc> {
 ...
 private:
  ...
  DISALLOW_EVIL_CONSTRUCTORS(UnweightedTester);
};
...
class AlgoTester {
 ...
 private:
  ...
  DISALLOW_EVIL_CONSTRUCTORS(AlgoTester);
};
