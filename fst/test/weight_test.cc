 ...
 private:
  ...
  void TestDivision(Weight w1, Weight w2) {
    ...
    if (Weight::Properties() & kLeftSemiring) {
      ...
      if (d.Member())
        CHECK(ApproxEqual(w2, d));
    }

    if (Weight::Properties() & kRightSemiring) {
      Weight d = Divide(p, w2, DIVIDE_RIGHT);
      if (d.Member())
        CHECK(ApproxEqual(w1, d));
    }
  ...
  DISALLOW_EVIL_CONSTRUCTORS(WeightTester);
};
...
int main(int argc, char **argv) {
  ...
  TropicalWeightGenerator tropical_generator(seed);
  WeightTester<TropicalWeight, TropicalWeightGenerator>
    tropical_tester(tropical_generator);
  tropical_tester.Test();

  LogWeightGenerator log_generator(seed);
  WeightTester<LogWeight, LogWeightGenerator>
