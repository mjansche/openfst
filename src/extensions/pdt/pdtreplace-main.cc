...
int pdtreplace_main(int argc, char **argv) {
  ...
  if (!s::GetPdtParserType(FST_FLAGS_pdt_parser_type, &parser_type)) {
    LOG(ERROR) << argv[0]
               << ": Unknown PDT parser type: " << FST_FLAGS_pdt_parser_type;
    return 1;
  }
  ...
  s::PdtReplace(s::BorrowPairs(pairs), &ofst, &parens, root, parser_type,
                ...
                FST_FLAGS_left_paren_prefix, FST_FLAGS_right_paren_prefix);

  if (!FST_FLAGS_pdt_parentheses.empty()) {
    if (!WriteLabelPairs(FST_FLAGS_pdt_parentheses, parens)) return 1;
  }
