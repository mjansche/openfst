...
int farcompilestrings_main(int argc, char **argv) {
  ...
  if (!s::GetFarEntryType(FST_FLAGS_entry_type, &entry_type)) {
    LOG(ERROR) << "Unknown or unsupported FAR entry type: " << FST_FLAGS_entry_type;
    return 1;
  }

  fst::TokenType token_type;
  if (!s::GetTokenType(FST_FLAGS_token_type, &token_type)) {
    LOG(ERROR) << "Unknown or unsupported FAR token type: " << FST_FLAGS_token_type;
    return 1;
  }
  ...
  const std::string fst_type =
      FST_FLAGS_fst_type.empty() ? "vector" : FST_FLAGS_fst_type;

  s::FarCompileStrings(in_sources, out_source, FST_FLAGS_arc_type, fst_type,
                       far_type, FST_FLAGS_generate_keys, entry_type,
                       token_type, FST_FLAGS_symbols, FST_FLAGS_unknown_symbol,
                       FST_FLAGS_keep_symbols,
