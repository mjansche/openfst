...
void RemoveWeight(MutableFst<Arc> *fst, typename Arc::Weight w, bool at_final) {
  if ((w == Arc::Weight::One()) || (w == Arc::Weight::Zero())) return;
  ...
  if (at_final) {
    ...
    for (MutableArcIterator<MutableFst<Arc>> ait(fst, start); !ait.Done();
         ait.Next()) {
      Arc arc = ait.Value();
...
void Push(const Fst<Arc> &ifst, MutableFst<Arc> *ofst, uint32 ptype,
  ...
  if ((ptype & (kPushWeights | kPushLabels)) == kPushWeights) {
    *ofst = ifst;
    ...
    const GallicType gtype =
        rtype == REWEIGHT_TO_INITIAL ? GALLIC_LEFT : GALLIC_RIGHT;
    std::vector<typename GallicArc<Arc, gtype>::Weight> gdistance;
    ...
    if (ptype & (kPushRemoveTotalWeight | kPushRemoveCommonAffix)) {
      ...
      total_weight = typename GallicArc<Arc, gtype>::Weight(
          ptype & kPushRemoveCommonAffix
              ? total_weight.Value1()
              : StringWeight<typename Arc::Label,
                             GALLIC_STRING_TYPE(gtype)>::One(),
